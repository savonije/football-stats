...
- **Dialog width is set exactly one way:** a Tailwind container width class through `UModal`'s `ui` prop — `:ui="{ content: 'w-md' }"` (the `w-3xs … w-7xl` scale). Never `style="width: 450px"`, `:style="{ width: … }"`, `w-96`, or `w-[400px]`. The `max-w-[calc(100vw-2rem)]` cap for small screens is set once in `vite.config.ts` under `ui.modal.variants.fullscreen.false.content`, so no responsive variant is needed — it has to sit on that variant rather than on `slots.content`, because the theme's own `max-w-lg` lives there and tailwind-merge lets the later class win.

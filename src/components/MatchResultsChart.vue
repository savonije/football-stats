<script setup lang="ts">
    ...
    const colWidth = computed(() =>
        props.data.length > 0
            ? (containerWidth.value - PAD_L - PAD_R) / props.data.length
            : 0,
    );

    const maxGoalDiff = computed(() =>
        Math.max(
            ...props.data.map((d) => Math.abs(d.goalsFor - d.goalsAgainst)),
            1,
        ),
    );
...
<template>
    <div v-if="data.length === 0" class="py-4 text-center text-gray-400">
        {{ t('match.noMatches') }}
    </div>

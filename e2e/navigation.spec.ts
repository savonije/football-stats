...
test.describe('Navigation', () => {
    ...
    test('header shows club name and team', async ({ page }) => {
        await page.goto('/');
        await expect(
            page.getByRole('heading', { name: "SV Apollo '69" }),
        ).toBeVisible();
    });
    ...
    test('nav contains all main links', async ({ page }) => {
        ...
        await openMenu(page);
        await expect(
            page.getByRole('link', { name: 'Wedstrijden' }),
        ).toBeVisible();
        await expect(
            page.getByRole('link', { name: 'Toplijst' }),
        ).toBeVisible();
    ...
    test('navigates to players page', async ({ page }) => {
        ...
        await expect(
            page.getByRole('heading', { name: 'Spelers', exact: true }),
        ).toBeVisible();
    });
    ...
    test('navigates to topscorers page', async ({ page }) => {
        ...
        await expect(
            page.getByRole('heading', { name: 'Topscoorders' }),
        ).toBeVisible();
    });
